...
async def main() -> None:
    ...
    response = await read.user_twap_history.get_by_addr(sub_addr=SUB_ADDR, limit=10)

    if not response.items:
        print(f"No TWAP history for {SUB_ADDR}")
        return

    print(f"TWAP History for {SUB_ADDR}:\n")
    for twap in response.items:
